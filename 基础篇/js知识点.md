...
### NodeList对象转换为数组
  ```
    let arrayNodes = Array.prototype.slice.call(rootNode.childNodes, 0);
    hasChildNodes() //在节点包含一个或多个子节点的情况下返回true.
  ```
### 操作节点
  1. appendChild() //用于像childNodes末尾添加一个节点，返回新增的节点, 如果节点已存在，则移动到新位置
  2. insertBefore() // 节点插入指定位置
  3. replaceChild() // 替换指定的节点
  4. removeChild() // 移除节点
  5. cloneChild([true]) //参数为true，深复制, false时复制节点本身, 建议复制之前最好先移除事件处理程序

### 访问节点
  Node.firstChild[lastChild | parentChild | nextSibling | previousSibling]

### DOM扩展
  1. querySelector() // 参数为css选择符
  2. querySelectorAll() // 返回所有匹配的元素
  3. childElementCount/firstElementChild/lastElementChild/lastElementChild/previousElementSibling/nextElentSibling
...
```
  ...
  var p1 = new Person({name: '123', age: 18});
  function Person1(obj) {
    Person.call(this, obj);
    this.sex = obj.sex;
  }
  Person1.prototype = Object.create(Person.prototype);
  Person1.prototype.play = fucntion (value) {
    console.log(value);
  }
  var p2 = new Person1({name: '123', age: 18, sex:'mam'});
```